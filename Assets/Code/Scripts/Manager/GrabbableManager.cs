...
public class GrabbableManager : MonoBehaviour
{
    ...
    {
        if (lastMaterial != targetMaterial)
        { // If the material is not the target material
            if (lastMaterial == 3 || targetMaterial == 3)
            { //if it was transparent before, instantly change it to the target material, as transparency is not lerpable
            ...
            else
            {
                GetComponent<Renderer>()
                    .material.Lerp(
                        ...
                        GetMaterialById(targetMaterial),
                        0.1f
                    );
                if (
        ...
        if (LevelManager.isDebug && isGrabbed)
        {
            ...
            Vector3Int grabbableGridSize = new Vector3Int(
                grabbable.size[0],
                grabbable.size[1],
                grabbable.size[2]
            );
            ...
            (int rotatedX, int rotatedY, int rotatedZ) = RotationHelper.RotateDimensionSize(
                grabbableGridSize.x,
                grabbableGridSize.y,
                grabbableGridSize.z,
...
Rotated Grid Size: {Vector3Int.RoundToInt(rotatedGridSize)}";
    ...
    public void OnSelectEnter(SelectEnterEventArgs args)
    {
        ...
        {
            FillableManager fillableManager =
                insideFillable.fillableObject.GetComponent<FillableManager>();
